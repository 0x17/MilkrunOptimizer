...
namespace MilkrunOptimizer.Cluster {
    public class MergeResultFiles {
        public static TrainingData MergeDataInPath(string path, string extension, bool showProgress = true) {
            ...
            var trainingDatas = filenames.Select(TrainingDataPersistence.LoadFromDisk);
            ...
            foreach (var td in trainingDatas) {
                if (showProgress)
                    Console.WriteLine(
                        $"Adding {td.Samples.Count} samples to the merged dataset. Progress is {(float) ctr / (float) filenames.Count * 100.0f}%...");
